...
for filename in requirements.txt vncsock.py vnc.py pvevncproxy.sh pvevncproxyapi.sh app .gitignore
do
	sudo cp -r $filename /etc/pvevncproxy/
done

for filename in nodes.yaml uservmlist.yaml
do
    if [ ! -f /etc/pvevncproxy/$filename ]
    then
	sudo cp -r $filename /etc/pvevncproxy/
    fi
done
...
do
	sudo chmod +x /etc/pvevncproxy/$filename
done
